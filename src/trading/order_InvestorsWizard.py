"""Module for Order for Investors Wizard (Meisha)."""
...
class Order_InvestorsWizard(Order):
    """Order from InvestorsWizard (Meisha)."""
    ...
    def fit(self, text: str) -> None:
        ...
        if len(words) == 0 or len(words) == 1:
            ...
            self.set_call(words, is_entry_order)
        elif words[1] == "UPDATE:":
            self.segment = words[0]
            self.set_update(words)
        else:
    ...
    def set_call(self, words: List[str], is_entry_order: bool) -> None:
        ...
        if words[4] in ["ABOVE", "BELOW"]:
            self.type = "entry" if is_entry_order else "market"
            self.EPs = [float(words[5])]
        elif words[4] in ["AT"]:
            if words[5].isnumeric():
                self.type = "market"
                self.CMP = float(words[5])
                print(f"self.CMP={self.CMP}")
            elif "-" in words[5]:
        ...
        if words[8] == "/":
            # there is a TP2
            self.TPs.append(float(words[9]))
            self.SL = float(words[11])
        else:
            # there is no TP2
